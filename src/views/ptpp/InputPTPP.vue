<template>
  <v-container fluid>
    ...
    <v-card class="px-8" color="white">
      <v-row class="py-6" v-if="fromEdit">
        <v-col>
          <v-progress-linear :value="progressVal"></v-progress-linear>
        </v-col>
      </v-row>
      <v-row class="pt-2">
        <v-col cols="9">
...
export default {
    ...
    initProgressBar() {
      if (this.data.status != undefined) {
        switch (this.data.status) {
          case 'Menunggu Penerima PTPP':
            this.progressVal = 25;
            this.isMenungguPenerima = true;
            break;
          case 'Menunggu Feedback Pemohon':
            this.progressVal = 50;
            this.isMenungguFeedbackPemohon = true;
            break;
          case 'Menunggu Approver':
            this.progressVal = 75;
            this.isMenungguApprover = true;
            break;
          case 'Approved':
  ...
  beforeMount() {
    ...
    this.fromInput = this.data.fromInput;
