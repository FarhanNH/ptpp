    ...
    <v-card class="pl-8" color="white">
      <v-row class="pt-2">
        ...
        <v-col cols="5" class="d-flex justify-end pb-0">
          <v-btn color="primary" dark @click="addItem()"> Tambah PTPP </v-btn>
        </v-col>
      </v-row>
      <!-- <v-row>
        <v-col cols="6">
...
export default {
  ...
  methods: {
    ...
    movePage(val) {
      ...
      Router.redirectParam('Input PTPP', strVal);
    },
    dummy() {
      ...
      for (let i = 1; i < 7; i++) {
        if (i < 3) {
          ...
          this.dataTable.push({
            ...
            ptppNumber: '00' + i + '/PND648000/' + year,
            from: 'QQ',
            status: 'Menunggu Feedback Pemohon',
            edit: true,
          });
        } else {
          this.dataTable.push({
